...
public:
	...
	void proceseaza() {
		const int deaccelerare = 1;



		pozitie.y -= deaccelerare;
	...
	void deseneaza() {
		...
		glLoadIdentity();//matrice unitate

		glPushMatrix();//Tine minte ce matrici au fost initializate pana acum,tot ce va fi adaugat de aici incolo va putea fi resetat cu glPopMatrix()
		//Transformari
		//Transformarile de aici se compun
		glTranslated(pozitie.x, pozitie.y, 0);//translatie masina
		MasinaPolitie.CameraTranslation();//translatie camera
		glRotated(this->rotatie, 0, 0, 1);//rotatie masina
		
		//Primitive
		//Sasiu
		glColor3f(0.0, 0, 0);//Culoare
		glRectf(30, -50, -30, 50);
